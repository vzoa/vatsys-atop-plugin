...
/// <summary>
/// North Atlantic minima per ATOP NAS-MD-4714 and NAT Doc 007
/// </summary>
public class NorthAtlanticMinimaDelegate : IMinimaDelegate
{
    ...
    private const int StandardLateral = 60;  // SLOP tracks (1nm offset) - reduced from 120
    private const int NonRnpLateral = 120;   // Non-RNP equipped
    ...
    private const int Above600Vertical = 5000;   // Above FL600
    ...
    private const int TimeLongitudinalOpposite = 10;   // Opposite direction (for climb/descent separation)
    ...
    public int GetLateralMinima(FDP2.FDR fdr1, FDP2.FDR fdr2)
    {
        ...
        // SLOP with adjacent track - 60nm reduced
        if (IsSlopCapable(fdr1) && IsSlopCapable(fdr2))
            return StandardLateral;

        // Non-RNP equipped - 120nm
        return NonRnpLateral;
    }
    ...
    public int? GetLongitudinalDistanceMinima(FDP2.FDR fdr1, FDP2.FDR fdr2)
    {
        ...
        // Distance separation not applicable without proper equipment
        return null;
    }

    /// <summary>
    /// Determine track type (same, opposite, crossing) per NAS-MD-4714 Appendix A.3.82 DIR_TYPE
    /// </summary>
    ...
    /// Per NAS-MD-4714 Section 6.2.4.3
    /// </summary>
    private static bool CanApplyMnt(FDP2.FDR fdr1, FDP2.FDR fdr2)
    {
        // MNT requires:
        // 1. Both aircraft must be jets operating at same level
        // 2. Leading aircraft must maintain faster or same Mach
        // 3. Aircraft must be ADS-C equipped or position reports available
    ...
    /// <summary>
    /// Check if aircraft is SLOP (Strategic Lateral Offset Procedure) capable
    /// </summary>
    private static bool IsSlopCapable(FDP2.FDR fdr)
    {
        // SLOP capable if RNP equipped (can hold precise offset)
        return CanApplyRnp10(fdr) || CanApplyRnp4(fdr);
    }
    ...
    {
        // Concorde retired, but future supersonic aircraft may exist
        return false;
    }

    /// <summary>
    /// Check if aircraft has DME capability for distance-based separation
    /// </summary>
    private static bool HasDme(FDP2.FDR fdr)
    {
        // Check for DME in equipment code (D)
